...
impl Client {
    ...
    pub async fn get_rooms(&self) -> Result<String> {
        self.get("/rooms").await
    }

    pub async fn add_room(&self, name: String) -> Result<String> {
        self.post("/rooms").await
    }

    pub async fn get_room(&self, id: String) -> Result<String> {
        let path = format!("/rooms/{id}");
        self.get(&path).await
    }

    pub async fn update_room(&self, id: String, name: String) -> Result<String> {
        let path = format!("/rooms/{id}");
    ...
    async fn patch(&self, path: &str) -> Result<String> {
        let url = self.make_url(path);
